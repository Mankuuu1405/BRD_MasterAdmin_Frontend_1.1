...
export default function RiskMitigationRuleList(){
  ...
  const [rules,setRules]=useState([
    {id:1,risk:"High FOIR",mitigation:"Collateral Required",severity:"High",status:"Active"},
    {id:2,risk:"Low Credit Score",mitigation:"Guarantor Mandatory",severity:"Medium",status:"Active"},
  ]);

  const filtered=rules.filter(r=>r.risk.toLowerCase().includes(search.toLowerCase()));
    ...
    <MainLayout>
      ...
      <div className="space-y-3">
        <div className="hidden md:grid grid-cols-5 bg-gray-100 rounded-xl px-5 py-3 text-xs font-semibold text-gray-600">
          <div>Risk Parameter</div><div>Mitigation Action</div><div>Severity</div><div>Status</div>
