...
export default function ViewInternalScoreRule(){
  ...
  useEffect(()=>{
    setRule({parameter:"Bank Balance",min_value:10000,weight:20,risk:"Low",status:"Active",remarks:"Standard rule"});
  },[id]);
  ...
  return (
    <MainLayout>
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={()=>navigate(`/rule-management/scorecard/internal/edit/${id}`)} className="px-4 py-2 bg-indigo-600 text-white rounded-xl flex items-center gap-2">
          <FiEdit3/> Edit
        </button>
      </div>
...
const Info=({label,value})=>(
  <div><label className="text-sm font-medium text-gray-600">{label}</label><p className="mt-1 text-sm text-gray-900">{value}</p></div>
);
